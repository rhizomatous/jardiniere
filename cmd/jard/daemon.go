...
func newDaemonCmd(g *globals) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: "jard runs a small daemon that owns sandbox state and lifecycle. It starts " +
...
func newDaemonStopCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			_, err := lipgloss.Fprintln(cmd.OutOrStdout(), ui.Faint.Render("stopped the jard daemon"))
			return err
		},
	}
}
...
func report(cmd *cobra.Command, style lipgloss.Style, verb string, pid int) error {
	line := style.Render(verb) + ui.Faint.Render(" the jard daemon")
	if pid != 0 {
