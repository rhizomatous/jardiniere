// Command jardd is jard's host-resident daemon.
//
// It owns sandbox state and lifecycle, and serves them over a unix socket. It
// is normally started on demand by jard rather than run by hand; running it in
...
// version is set at build time, and must match the jard that talks to it.
var version = "dev"
...
func run() int {
	...
	// SIGTERM is how `jard daemon stop` asks; SIGINT is how a terminal does.
